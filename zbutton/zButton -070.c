...
//	unsigned int MAP_SIZE_SRC = 2*m_image_x*m_image_y;
	unsigned int MAP_SIZE_JPG = 1024*1024;//1MB buffer for jpg image
		...
		unsigned short motion_fs = 2*160;//2*80;//fixed,80 samples/second
		...
		motion_6axis = global_options->MOTION_6AXIS;
		...
		motion_size = (12*motion_fs);//(24*motion_fs);//24 bytes for each sample
...
//sprintf(HrMinSec,"%s%s%s",time_struct.time_hour,time_struct.time_min, time_struct.time_sec);		
	...
	int MOTION_save_sixaxis(char *imFile, unsigned short motion_fs, short *start_motion, char header)
	{
		...
		if(header)
			fprintf(fp, "TEMP,GYRO_X,GYRO_Y,GYRO_Z, time(hhmmss)\n");
...
//			MPU9150_DATA.ASA_Z = (char)(((*(start_motion+8))>>8)&0xff);
			...
			MPU9150_DATA.TEMP = ((float)(*(start_motion+3))/340.0+35);
			MPU9150_DATA.GYRO_X = 500.0*((float)(*(start_motion+2))/32767.0);
			MPU9150_DATA.GYRO_Y = 500.0*((float)(*(start_motion+1))/32767.0);
			MPU9150_DATA.GYRO_Z = 500.0*((float)(*(start_motion))/32767.0);

			start_motion = start_motion + 4;			
	
	
			fprintf(fp, "%9f,%9f,%9f,%9f\n,%s\n", MPU9150_DATA.TEMP, MPU9150_DATA.GYRO_X, MPU9150_DATA.GYRO_Y, MPU9150_DATA.GYRO_Z, time_print);
...
//			MPU9150_DATA.ASA_Z = (char)(((*(start_motion+8))>>8)&0xff);
			...
			MPU9150_DATA.ACC_X = 4.0*9.81*((float)(*(start_motion+3))/32767.0);
			MPU9150_DATA.ACC_Y = 4.0*9.81*((float)(*(start_motion+2))/32767.0);
			MPU9150_DATA.ACC_Z = 4.0*9.81*((float)(*(start_motion+1))/32767.0);
			MPU9150_DATA.TEMP = ((float)(*(start_motion))/340.0+35);
			MPU9150_DATA.GYRO_X = 500.0*((float)(*(start_motion+7))/32767.0);//*(start_motion+7);
			MPU9150_DATA.GYRO_Y = 500.0*((float)(*(start_motion+6))/32767.0);//*(start_motion+6);
			MPU9150_DATA.GYRO_Z = 500.0*((float)(*(start_motion+5))/32767.0);//*(start_motion+5);
			MPU9150_DATA.ASA_X = (char)(((*(start_motion+9))>>8)&0xff);
			MPU9150_DATA.ASA_Y = (char)((*(start_motion+9))&0xff);
			MPU9150_DATA.ASA_Z = (char)(((*(start_motion+8))>>8)&0xff);
