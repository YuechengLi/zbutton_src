...
//	unsigned int MAP_SIZE_SRC = 2*m_image_x*m_image_y;
	unsigned int MAP_SIZE_JPG = 1024*1024;//1MB buffer for jpg image
		...
		unsigned short motion_fs = 2*100;//2*80;//fixed,80 samples/second
		...
		motion_size = (16*motion_fs);//(24*motion_fs);//24 bytes for each sample
...
//			MPU9150_DATA.ASA_Z = (char)(((*(start_motion+8))>>8)&0xff);
			...
			MPU9150_DATA.TEMP = ((float)(*(start_motion))/340.0+35);
			MPU9150_DATA.GYRO_X = 500.0*((float)(*(start_motion+7))/32768.0);//*(start_motion+7);
			MPU9150_DATA.GYRO_Y = 500.0*((float)(*(start_motion+6))/32768.0);//*(start_motion+6);
			MPU9150_DATA.GYRO_Z = 500.0*((float)(*(start_motion+5))/32768.0);//*(start_motion+5);
